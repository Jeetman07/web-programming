document.addEventListener("DOMContentLoaded", () => {
    ...
    let numMovies = Number(prompt("How many movies do you want to rate?"));
    let movies = [];

    for (let i = 1; i <= numMovies; i++) {
        let title = prompt(`Enter the title of movie ${i}:`).toUpperCase();
        let rating;

        while (true) {
            rating = Number(prompt(`Enter your rating for "${title}" (1 to 5):`));
            if (!isNaN(rating) && rating >= 1 && rating <= 5) break;
